...
// Toggle navigation search bar 
var isSearchNavActive = false;
function toggleSearchInNav() {
    console.log('search toggle');
    $('#nav-search-field').toggleClass('expand');
    isSearchNavActive = !isSearchNavActive;
    console.log(isSearchNavActive);
    if (isSearchNavActive) {
        $('.nav-search-results').show();
        $('.site-content').on('click', function (event) {
            $('#nav-search-field').removeClass('expand');
            isSearchNavActive = false;
